...
struct SDCard<Content: View>: View {
    ...
    var body: some View {
        content().padding(16).background(SDTheme.panel).clipShape(RoundedRectangle(cornerRadius: 17, style: .continuous)).overlay(RoundedRectangle(cornerRadius: 17).stroke(SDTheme.line))
    }
}
...
struct AvatarView: View {
    ...
    var body: some View {
        ...
        }.frame(width: size, height: size).overlay(Circle().stroke(Color.white.opacity(0.12)))
    }
}
...
struct TagPill: View {
    ...
    var body: some View {
        Text(text).font(.caption2.weight(.semibold)).padding(.horizontal, 9).padding(.vertical, 5).foregroundStyle(highlighted ? .white : SDTheme.muted).background(highlighted ? SDTheme.accent : Color.white.opacity(0.06)).clipShape(Capsule())
    }
}
