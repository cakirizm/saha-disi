...
final class AppStore: ObservableObject {
    ...
    func statements(for commentatorID: String) -> [Statement] { (payload?.statements ?? []).filter { $0.commentator == commentatorID }.sorted { $0.date > $1.date } }
    func statements(team: String) -> [Statement] { (payload?.statements ?? []).filter { $0.team == team }.sorted { $0.date > $1.date } }
    func statements(player: String) -> [Statement] { (payload?.statements ?? []).filter { $0.players.contains(player) }.sorted { $0.date > $1.date } }
    func statements(matchID: String) -> [Statement] { (payload?.statements ?? []).filter { $0.matchId == matchID }.sorted { $0.date > $1.date } }
    ...
    var rankedCommentators: [(Commentator, Int)] { (payload?.commentators ?? []).map { ($0, statements(for: $0.id).count) }.sorted { $0.1 > $1.1 } }
    ...
    var hotStatements: [Statement] {
        (payload?.statements ?? []).sorted { $0.strength == $1.strength ? $0.date > $1.date : $0.strength > $1.strength }
    }
}
